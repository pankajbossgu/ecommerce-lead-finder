...
async function loadLeads(view = 'pending') { const target = view === 'pending' ? '#new-results' : `#${view}-results`; const search = view === 'pending' ? '' : $(`#${view}-search`).value.trim(); if (state.loading[view]) return; state.loading[view] = true; if (view !== 'pending') $(target).innerHTML = loading('Loading leads…'); try { const data = await api(`/api/leads?status=${view}&page=${state.pages[view] || 1}&limit=25&search=${encodeURIComponent(search)}`); if (view === 'pending') { state.pendingCount = data.pagination.total; $('#results-summary').textContent = state.pendingCount ? `${plural(state.pendingCount)} need your review.` : 'No leads waiting for review.'; updateLock(); } else $(`#${view}-count`).textContent = plural(data.pagination.total); const titles = { pending: ['No pending leads', 'Run a discovery search to begin.'], saved: ['No saved leads yet', 'Saved businesses will appear here.'], discarded: ['No excluded leads', 'Businesses marked Not Useful appear here.'] }; $(target).innerHTML = data.items.length ? leadRows(data.items, view) + pager(data, view) : empty(...titles[view]); updateToolbar(); } catch { $(target).innerHTML = empty('Unable to load leads', 'Please try again in a moment.'); } finally { state.loading[view] = false; } }
...
async function applyJob(job, jobId, version) {
  ...
  setProgress(job);
  await loadLeads();
  toast(job.status === 'cancelled' ? (state.pendingCount ? `Search cancelled. ${plural(state.pendingCount)} remain.` : 'Search cancelled') : job.status === 'failed' ? 'Search failed' : 'Search completed');
  return true;
}
