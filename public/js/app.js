...
function updateLock(job = state.currentJob) { state.searchLocked = Boolean(job && active(job.status)) || state.pendingCount > 0; if (job && active(job.status)) lockForm(true, 'Search in progress. Please wait for it to finish.'); else if (state.pendingCount) lockForm(true, 'Review the remaining leads: Save, mark Not Useful, or Clear them before starting a new search.'); else lockForm(false, 'Ready for a new search.'); }
function setStatusBanner(job = state.currentJob) { const banner = $('#discovery-status-banner'); const action = $('#status-primary-action'); if (!job) { banner.hidden = true; return; } const pending = Number(state.pendingCount) || 0; const outcome = active(job.status) ? 'in-progress' : `${job.status}-${pending ? 'pending' : 'empty'}`; banner.hidden = false; banner.dataset.outcome = outcome; $('#discovery-status-chip').textContent = active(job.status) ? '● Searching' : job.status === 'cancelled' ? '× Cancelled' : job.status === 'completed' ? '✓ Complete' : '!' + ' Needs attention'; $('#discovery-status-title').textContent = active(job.status) ? 'Discovery is in progress' : pending ? `${plural(pending)} ready for review` : job.status === 'cancelled' ? 'Search cancelled with no leads' : 'Discovery complete'; $('#discovery-status-message').textContent = active(job.status) ? 'Progress is saved and will continue to restore after refresh.' : pending ? 'Save, exclude, or clear the remaining leads to unlock a new search.' : 'You can start a new discovery search whenever you are ready.'; action.hidden = !pending; action.textContent = pending ? 'Review leads' : ''; }
...
async function restoreCurrent() {
  stopPolling(); state.jobVersion += 1;
  const current = await api('/api/discovery/current');
  state.cancelling = false;
  if (current.jobId) {
    state.jobId = current.jobId; state.currentJob = current; state.pendingCount = Number(current.pendingCount) || 0;
    $('#job-progress').hidden = !active(current.status);
    if (active(current.status)) { setProgress(current); startPolling(); }
  } else { state.jobId = null; state.currentJob = null; state.pendingCount = 0; $('#job-progress').hidden = true; }
  updateLock(); setStatusBanner(); await loadLeads();
}
async function applyJob(job, jobId, version) {
  if (state.jobId !== jobId || state.jobVersion !== version) return false;
  state.currentJob = job; state.pendingCount = Number(job.pendingCount ?? state.pendingCount) || 0;
  if (active(job.status)) { $('#job-progress').hidden = false; setProgress(job); updateLock(job); setStatusBanner(job); return true; }
  // Terminal sessions retain their id for review actions, but never their spinner.
  stopPolling(); state.cancelling = false; $('#job-progress').hidden = true;
  updateLock(job); setStatusBanner(job);
  await loadLeads('pending', { cancelled: job.status === 'cancelled' });
  toast(job.status === 'cancelled' ? (state.pendingCount ? `Search cancelled. ${plural(state.pendingCount)} remain for review.` : 'Search cancelled') : job.status === 'failed' ? 'Search failed' : 'Search completed');
  return true;
}
...
$('#save-selected').addEventListener('click', () => bulk('saved', [...state.selected])); $('#discard-selected').addEventListener('click', () => bulk('discarded', [...state.selected])); $('#clear-remaining').addEventListener('click', openClearModal); $('#clear-cancel').addEventListener('click', closeClearModal); $('#status-primary-action').addEventListener('click', () => { document.querySelector('.results-section').scrollIntoView({ behavior: 'smooth', block: 'start' }); }); $('#clear-confirm').addEventListener('click', async () => { const id = state.jobId; if (!id) return closeClearModal(); $('#clear-confirm').disabled = true; try { const result = await api(`/api/discovery/jobs/${id}/pending-leads`, { method: 'DELETE' }); state.pendingCount = Math.max(0, state.pendingCount - result.deletedCount); state.selected.clear(); closeClearModal(); await restoreCurrent(); toast(`✓ Cleared ${plural(result.deletedCount)}`); } catch (error) { toast(error.message); } finally { $('#clear-confirm').disabled = false; } });
