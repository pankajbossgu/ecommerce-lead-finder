...
import { createRfcMessageId, mailboxInput, messageIds, normalizedMessageId, persistCampaignMailboxEmail, persistReceived, receiveEmail, sendBrevoMailboxEmail, sendMailboxEmail, verifyResendWebhook } from './services/inbox.js';
...
const emailProvider = value => { const provider = value || 'resend'; if (!['resend', 'brevo'].includes(provider)) throw new AppError('Choose Resend or Brevo.', 400, 'VALIDATION_ERROR'); return provider; };
...
app.post('/api/mailbox/send', mailboxRateLimit, async (req, res) => { const input = mailboxInput(req.body); const provider = emailProvider(req.body?.emailProvider); res.status(201).json(await (provider === 'brevo' ? sendBrevoMailboxEmail(input) : sendMailboxEmail(input))); });
...
async function processEmailBatch(campaign, retryFailed = false) {
  ...
  try {
    ...
    const results = valid.length ? (campaign.emailProvider === 'brevo' ? await sendBrevoEmailBatch(outbound.map(item => ({ to: item.lead.email, subject: item.subject, text: item.text, headers: item.headers }))) : await sendEmailBatch(outbound.map(item => ({ to: item.lead.email, subject: item.subject, text: item.text, headers: item.headers })), batchKey)) : [];
