...
async function processEmailBatch(campaign, retryFailed = false) {
  ...
  try {
    ...
    const outbound = valid.map(r => { const lead = leads.get(String(r.leadId)); const subject = renderTemplate(template.subject, lead); const text = renderTemplate(template.body, lead); const messageId = createRfcMessageId(campaign.emailProvider === 'brevo' ? env.brevoEmailFrom : (env.resendEmailFrom || env.emailFrom)); return { recipient: r, lead, subject, text, messageId, headers: { 'Message-ID': messageId } }; });
    const results = valid.length ? (emailProvider(campaign.emailProvider) === 'brevo' ? await sendBrevoEmailBatch(outbound.map(item => ({ to: item.lead.email, subject: item.subject, text: item.text, headers: item.headers, ...(env.emailReplyTo ? { replyTo: env.emailReplyTo } : {}) }))) : await sendEmailBatch(outbound.map(item => ({ to: item.lead.email, subject: item.subject, text: item.text, headers: item.headers, ...(env.emailReplyTo ? { replyTo: env.emailReplyTo } : {}) })), batchKey)) : [];
