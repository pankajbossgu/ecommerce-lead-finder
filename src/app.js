...
import { sendEmailBatch, validateEmailProvider } from './services/email.js';
...
const emailProvider = validateEmailProvider;
...
export function managementPipeline(query) {
  ...
  pipeline.push({ $addFields: {
    emailStatus: { $switch: { branches: [{ case: { $in: ['$emailActivity.status', ['sent', 'manual_sent']] }, then: 'sent' }, { case: { $eq: ['$emailActivity.status', 'failed'] }, then: 'failed' }, { case: { $eq: ['$emailActivity.status', 'skipped'] }, then: 'skipped' }], default: 'not_sent' } },
    whatsappStatus: { $switch: { branches: [{ case: { $in: ['$whatsappActivity.status', ['sent', 'manual_sent']] }, then: 'sent' }, { case: { $eq: ['$whatsappActivity.status', 'failed'] }, then: 'failed' }, { case: { $eq: ['$whatsappActivity.status', 'skipped'] }, then: 'skipped' }], default: 'not_sent' } }
  } });
...
async function matchImportedRow(row) {
  ...
  // Each level is deliberately checked on its own. Combining identifiers in an
  // $or made a single CSV row select every matching Saved lead.
  const find = query => Lead.find(query).sort({ _id: 1 }).lean();
  let candidates = [];
  if (row.id && mongoose.isValidObjectId(row.id)) candidates = await find({ _id: row.id });
  if (!candidates.length) {
    const domain = normalizeDomain(row.website);
    if (domain) candidates = await find({ domain });
  }
  if (!candidates.length) {
    const email = normalizeEmail(row.email);
    if (email) candidates = await find({ $or: [{ emailNormalized: email }, { email }] });
  }
  if (!candidates.length) {
    const phone = normalizePhone(row.phone);
    if (phone) candidates = await find({ phone });
  }
  if (!candidates.length) {
    const name = normalizeBusinessName(row.business_name);
    if (!name) return { saved: [], discarded: [], selectedLeadId: null, multipleMatches: false };
    const byName = await find({ businessName: { $regex: `^${row.business_name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, $options: 'i' } });
    candidates = byName.filter(lead => normalizeBusinessName(lead.businessName) === name);
  }
  const saved = candidates.filter(lead => lead.status === 'saved').map(normalizedLead);
  return {
    saved,
    discarded: candidates.filter(lead => lead.status === 'discarded').map(normalizedLead),
    // A Saved lead is selectable only when this matching level identifies one.
    selectedLeadId: saved.length === 1 ? saved[0]._id : null,
    multipleMatches: saved.length > 1
  };
}
app.post('/api/leads/csv/preview', async (req, res) => {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (mode === 'new') {
      ...
      if (matches.multipleMatches) results.push({ row: rowNumber, status: 'multiple_matches', savedLeads: matches.saved, notUsefulLeads: matches.discarded, error: 'Multiple Lead Management records match this row. Review required.' });
      else if (matches.saved.length && matches.discarded.length) results.push({ row: rowNumber, status: 'found_in_both', savedLeads: matches.saved, notUsefulLeads: matches.discarded, selectedLeadId: matches.selectedLeadId });
      else if (matches.saved.length) results.push({ row: rowNumber, status: 'found_in_lead_management', savedLeads: matches.saved, selectedLeadId: matches.selectedLeadId });
...
function templateInput(body) {
  ...
  if (!['email', 'whatsapp'].includes(type)) throw new AppError('Template type must be email or whatsapp', 400, 'VALIDATION_ERROR');
  const subject = type === 'email' ? cleanText(body.subject, 'Subject', 200) : null;
  if (type !== 'email') return { name: cleanText(body.name, 'Template name', 120), type, subject, body: cleanText(body.body ?? body.message, 'Message', 10000), format: 'plain', htmlBody: null };
  const format = body?.format === undefined ? 'plain' : body.format;
  if (!['plain', 'html'].includes(format)) throw new AppError('Email format must be plain or html', 400, 'VALIDATION_ERROR');
  const textBody = cleanText(body.textBody ?? body.body ?? body.message, format === 'html' ? 'Plain-text fallback' : 'Message', 10000, format !== 'html');
  if (format === 'plain') return { name: cleanText(body.name, 'Template name', 120), type, subject, body: textBody, format, htmlBody: null };
  const htmlBody = cleanText(body.htmlBody, 'HTML body', 200000);
  if (!/<[a-z][\s\S]*>/i.test(htmlBody)) throw new AppError('HTML body must contain email markup.', 400, 'VALIDATION_ERROR');
...
app.get('/api/templates', async (_req, res) => res.json({ items: (await OutreachTemplate.find({}).sort({ updatedAt: -1 }).lean()).map(templateResponse) }));
app.post('/api/templates', async (req, res) => res.status(201).json(templateResponse((await OutreachTemplate.create(templateInput(req.body))).toObject())));
app.get('/api/templates/:id', async (req, res) => { objectId(req.params.id, 'Template'); const item = await OutreachTemplate.findById(req.params.id).lean(); if (!item) throw new AppError('Template not found', 404, 'NOT_FOUND'); res.json(templateResponse(item)); });
app.patch('/api/templates/:id', async (req, res) => { objectId(req.params.id, 'Template'); const item = await OutreachTemplate.findByIdAndUpdate(req.params.id, templateInput(req.body), { new: true, runValidators: true }).lean(); if (!item) throw new AppError('Template not found', 404, 'NOT_FOUND'); res.json(templateResponse(item)); });
...
app.post('/api/campaigns/:id/recipients', async (req, res) => {
  ...
  const saveBatch = async leads => {
    ...
    const docs = leads.flatMap(lead => campaign.channels.map(channel => ({ lead, channel })).filter(({ lead, channel }) => (channel === 'email' ? lead.email : lead.phone) && (req.body?.includeContacted === true || (channel === 'email' ? !contactedEmails.has(normalizeEmail(lead.email)) : !contacted.has(`${lead._id}:${channel}`)))).map(({ lead, channel }) => ({ campaignId: campaign._id, leadId: lead._id, channel, emailProvider: channel === 'email' ? emailProvider(campaign.emailProvider) : null, recipient: channel === 'email' ? lead.email : lead.phone, templateId: channel === 'email' ? campaign.emailTemplateId : campaign.whatsappTemplateId, status: 'ready', idempotencyKey: `campaign:${campaign._id}:lead:${lead._id}:channel:${channel}` })));
...
export async function resolveAmbiguousBrevoBatch(campaignId, recipientModel, activityModel, now = new Date()) {
  const filter = { campaignId, channel: 'email', status: 'sending', providerAcceptanceUnknown: true };
  const recipients = await recipientModel.find(filter).lean();
  if (!recipients.length) throw new AppError('This campaign has no Brevo batch awaiting manual resolution.', 409, 'NO_AMBIGUOUS_BATCH');
  const failureReason = 'EMAIL_PROVIDER_AMBIGUOUS: Manually resolved without resending because Brevo acceptance could not be confirmed.';
  await recipientModel.updateMany(filter, { $set: { status: 'failed', failedAt: now, failureReason, providerAcceptanceUnknown: false, sendingLeaseExpiresAt: null } });
  await Promise.all(recipients.map(recipient => activityModel.create({ leadId: recipient.leadId, campaignId, channel: 'email', emailProvider: 'brevo', templateId: recipient.templateId, recipient: recipient.recipient, status: 'failed', failedAt: now, failureReason })));
...
async function recordEmailFailure(recipient, campaign, lead, template, reason, code) {
  ...
  await Promise.all([
    ...
    OutreachActivity.create({ leadId: recipient.leadId, campaignId: campaign._id, channel: 'email', emailProvider: emailProvider(campaign.emailProvider), templateId: template?._id || recipient.templateId, recipient: lead?.email || recipient.recipient, subject: template && lead ? renderTemplate(template.subject, lead) : null, status: 'failed', failedAt, failureReason: `${code}: ${reason}`.slice(0, 500) })
  ]);
}
...
async function processEmailBatch(campaign, retryFailed = false) {
  ...
  try {
    ...
    const outbound = valid.map(r => { const lead = leads.get(String(r.leadId)); const subject = renderTemplate(template.subject, lead); const text = renderTemplate(template.body, lead); const html = template.format === 'html' && template.htmlBody ? renderTemplate(template.htmlBody, lead) : null; const messageId = createRfcMessageId(campaign.emailProvider === 'brevo' ? env.brevoEmailFrom : (env.resendEmailFrom || env.emailFrom)); return { recipient: r, lead, subject, text, html, messageId, headers: { 'Message-ID': messageId } }; });
    const results = valid.length ? (emailProvider(campaign.emailProvider) === 'brevo' ? await sendBrevoEmailBatch(outbound.map(item => ({ to: item.lead.email, subject: item.subject, text: item.text, ...(item.html ? { html: item.html } : {}), headers: item.headers, ...(env.emailReplyTo ? { replyTo: env.emailReplyTo } : {}) }))) : await sendEmailBatch(outbound.map(item => ({ to: item.lead.email, subject: item.subject, text: item.text, ...(item.html ? { html: item.html } : {}), headers: item.headers, ...(env.emailReplyTo ? { replyTo: env.emailReplyTo } : {}) })), batchKey)) : [];
    await Promise.all(valid.map(async (r, index) => {
      ...
      await Promise.all([CampaignRecipient.updateOne({ _id: r._id, status: 'sending', batchKey }, { $set: { status: 'sent', sentAt, providerMessageId: result.id, mailboxMessageId: sentMessage.messageId, mailboxSubject: sentMessage.subject, mailboxText: sentMessage.text, mailboxPersistencePending: false, sendingLeaseExpiresAt: null } }), OutreachActivity.create({ leadId: r.leadId, campaignId: campaign._id, channel: 'email', emailProvider: emailProvider(campaign.emailProvider), templateId: template._id, recipient: lead.email, subject: sentMessage.subject, status: 'sent', sentAt, providerMessageId: result.id })]);
...
app.post('/api/campaigns/:id/resolve-ambiguous-brevo-batch', async (req, res) => {
  const campaign = await requireCampaign(req.params.id);
  if (emailProvider(campaign.emailProvider) !== 'brevo') throw new AppError('Only Brevo batches can require provider acceptance resolution.', 409, 'NO_AMBIGUOUS_BATCH');
  if (req.body?.resolution !== 'mark_failed') throw new AppError('Resolve an ambiguous Brevo batch by explicitly marking it failed; this endpoint never resends email.', 400, 'VALIDATION_ERROR');
  const resolved = await resolveAmbiguousBrevoBatch(campaign._id, CampaignRecipient, OutreachActivity);
  const counts = await updateCampaignCounts(campaign._id);
  const status = counts.emailPending ? 'ready' : (counts.emailFailedCount && !counts.emailSentCount ? 'failed' : 'completed');
  await Campaign.findByIdAndUpdate(campaign._id, { $set: { status, currentBatchProcessed: resolved, ...(counts.emailPending ? {} : { completedAt: new Date() }) } });
  res.json({ resolved, counts, campaign: await requireCampaign(req.params.id) });
});
...
app.post('/api/campaigns/:id/recipients/:recipientId/skip', async (req, res) => { const recipient = await recipientFor(req.params.id, req.params.recipientId); const skipReason = skipReasonInput(req.body?.skipReason); const updated = await CampaignRecipient.findOneAndUpdate({ _id: recipient._id, status: { $in: ['ready', 'pending'] } }, { $set: { status: 'skipped', skipReason } }, { new: true }).lean(); if (updated) await OutreachActivity.create({ leadId: updated.leadId, campaignId: updated.campaignId, channel: updated.channel, templateId: updated.templateId, recipient: updated.recipient, status: 'skipped', skipReason }); res.json({ recipient: updated || recipient, counts: await updateCampaignCounts(req.params.id) }); });
