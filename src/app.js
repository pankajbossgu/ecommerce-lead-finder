...
async function updateCampaignCounts(campaignId) {
  const rows = await CampaignRecipient.aggregate([{ $match: { campaignId: new mongoose.Types.ObjectId(campaignId) } }, { $group: { _id: null, total: { $sum: 1 }, businesses: { $addToSet: '$leadId' }, email: { $sum: { $cond: [{ $eq: ['$channel', 'email'] }, 1, 0] } }, whatsapp: { $sum: { $cond: [{ $eq: ['$channel', 'whatsapp'] }, 1, 0] } }, emailSent: { $sum: { $cond: [{ $and: [{ $eq: ['$channel', 'email'] }, { $eq: ['$status', 'sent'] }] }, 1, 0] } }, emailFailed: { $sum: { $cond: [{ $and: [{ $eq: ['$channel', 'email'] }, { $eq: ['$status', 'failed'] }] }, 1, 0] } }, emailPending: { $sum: { $cond: [{ $and: [{ $eq: ['$channel', 'email'] }, { $in: ['$status', ['pending', 'ready', 'sending']] }] }, 1, 0] } }, whatsappSent: { $sum: { $cond: [{ $and: [{ $eq: ['$channel', 'whatsapp'] }, { $eq: ['$status', 'manual_sent'] }] }, 1, 0] } } } }]);
  const c = rows[0] || { total: 0, businesses: [], email: 0, whatsapp: 0, emailSent: 0, emailFailed: 0, emailPending: 0, whatsappSent: 0 };
  const processed = c.emailSent + c.emailFailed;
  await Campaign.findByIdAndUpdate(campaignId, { $set: { recipientCount: c.total, businessCount: c.businesses.length, emailCount: c.email, whatsappCount: c.whatsapp, emailSentCount: c.emailSent, emailFailedCount: c.emailFailed, emailPendingCount: c.emailPending, whatsappSentCount: c.whatsappSent, sentCount: c.emailSent + c.whatsappSent, failedCount: c.emailFailed, pendingCount: c.emailPending, processedCount: processed } });
...
function selectedLeadFilter(body) { if (body?.selectAllMatching === true) return leadFilter({ status: 'saved', search: body.search, from: body.from, to: body.to }, { allowJob: false }); const ids = Array.isArray(body?.leadIds) ? [...new Set(body.leadIds)] : []; if (!ids.length || ids.length > 100 || !ids.every(mongoose.isValidObjectId)) throw new AppError('Choose one to 100 valid saved leads', 400, 'VALIDATION_ERROR'); return { _id: { $in: ids }, status: 'saved' }; }
app.post('/api/campaigns/:id/recipients', async (req, res) => { const campaign = await requireCampaign(req.params.id); if (!['draft', 'ready', 'paused'].includes(campaign.status)) throw new AppError('Campaign cannot accept recipients while sending or completed', 409, 'CAMPAIGN_LOCKED'); const leads = await Lead.find(selectedLeadFilter(req.body)).lean(); if (!leads.length) throw new AppError('No saved leads match this selection', 409, 'LEADS_NOT_SAVED'); const activities = await OutreachActivity.find({ leadId: { $in: leads.map(l => l._id) }, status: { $in: ['sent', 'manual_sent'] } }).lean(); const contacted = new Set(activities.map(a => `${a.leadId}:${a.channel}`)); const docs = leads.flatMap(lead => campaign.channels.map(channel => ({ lead, channel })).filter(({ lead, channel }) => (channel === 'email' ? lead.email : lead.phone) && (req.body?.includeContacted === true || !contacted.has(`${lead._id}:${channel}`))).map(({ lead, channel }) => ({ campaignId: campaign._id, leadId: lead._id, channel, recipient: channel === 'email' ? lead.email : lead.phone, templateId: channel === 'email' ? campaign.emailTemplateId : campaign.whatsappTemplateId, status: 'ready', idempotencyKey: `campaign:${campaign._id}:lead:${lead._id}:channel:${channel}` })));
  const result = docs.length ? await CampaignRecipient.bulkWrite(docs.map(doc => ({ updateOne: { filter: { campaignId: doc.campaignId, leadId: doc.leadId, channel: doc.channel }, update: { $setOnInsert: doc }, upsert: true } }))) : { upsertedCount: 0 }; const counts = await updateCampaignCounts(campaign._id); await Campaign.findByIdAndUpdate(campaign._id, { $set: { status: counts.total ? 'ready' : 'draft' } }); res.status(201).json({ created: result.upsertedCount || 0, counts }); });
app.get('/api/campaigns/:id/recipients', async (req, res) => { const campaign = await requireCampaign(req.params.id); const items = await CampaignRecipient.find({ campaignId: campaign._id }).populate('leadId', 'businessName email phone domain').sort({ createdAt: -1 }).lean(); const grouped = new Map(); for (const item of items) { const key = String(item.leadId?._id || item.leadId); if (!grouped.has(key)) grouped.set(key, { lead: item.leadId, email: null, whatsapp: null, lastContacted: null }); const row = grouped.get(key); row[item.channel] = item; const date = item.sentAt || item.failedAt; if (date && (!row.lastContacted || date > row.lastContacted)) row.lastContacted = date; } res.json({ campaign, items: [...grouped.values()] }); });
async function claimEmailBatch(campaign) { const stale = await CampaignRecipient.findOne({ campaignId: campaign._id, channel: 'email', status: 'sending', sendingLeaseExpiresAt: { $lt: new Date() } }).lean(); if (stale?.batchKey) return { claimed: await CampaignRecipient.find({ campaignId: campaign._id, batchKey: stale.batchKey, status: 'sending' }).lean(), batchKey: stale.batchKey, batchNumber: stale.batchNumber || campaign.currentBatch }; const batchKey = `campaign:${campaign._id}:batch:${crypto.randomUUID()}`, batchNumber = (campaign.currentBatch || 0) + 1, claimed = []; for (let i = 0; i < 100; i += 1) { const item = await CampaignRecipient.findOneAndUpdate({ campaignId: campaign._id, channel: 'email', status: { $in: ['ready', 'pending', 'failed'] } }, { $set: { status: 'sending', failureReason: null, batchKey, batchNumber, sendingLeaseExpiresAt: new Date(Date.now() + 10 * 60_000) }, $inc: { attempts: 1 } }, { new: true }).lean(); if (!item) break; claimed.push(item); } if (claimed.length) await Campaign.findByIdAndUpdate(campaign._id, { $set: { status: 'sending', currentBatch: batchNumber, startedAt: campaign.startedAt || new Date(), completedAt: null } }); return { claimed, batchKey, batchNumber }; }
async function processEmailBatch(campaign) { const { claimed, batchKey, batchNumber } = await claimEmailBatch(campaign); if (!claimed.length) return { processed: 0 }; const leads = new Map((await Lead.find({ _id: { $in: claimed.map(r => r.leadId) }, status: 'saved' }).lean()).map(l => [String(l._id), l])); const template = await OutreachTemplate.findOne({ _id: campaign.emailTemplateId, type: 'email' }).lean(); const valid = claimed.filter(r => leads.has(String(r.leadId)) && template && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(leads.get(String(r.leadId)).email));
  for (const r of claimed.filter(r => !valid.includes(r))) await CampaignRecipient.updateOne({ _id: r._id, status: 'sending' }, { $set: { status: 'failed', failedAt: new Date(), failureReason: 'Lead, email, or template is unavailable' } });
  try { const ids = await sendEmailBatch(valid.map(r => { const lead = leads.get(String(r.leadId)); return { to: lead.email, subject: renderTemplate(template.subject, lead), text: renderTemplate(template.body, lead) }; }), batchKey); const sentAt = new Date(); await Promise.all(valid.flatMap((r, index) => [CampaignRecipient.updateOne({ _id: r._id, status: 'sending', batchKey }, { $set: { status: 'sent', sentAt, providerMessageId: ids[index]?.id || ids[index] || null, sendingLeaseExpiresAt: null } }), OutreachActivity.create({ leadId: r.leadId, campaignId: campaign._id, channel: 'email', templateId: template._id, recipient: leads.get(String(r.leadId)).email, subject: renderTemplate(template.subject, leads.get(String(r.leadId))), status: 'sent', sentAt, providerMessageId: ids[index]?.id || ids[index] || null })])); } catch (error) { await CampaignRecipient.updateMany({ campaignId: campaign._id, batchKey, status: 'sending' }, { $set: { status: 'failed', failedAt: new Date(), failureReason: error.message.slice(0, 500), sendingLeaseExpiresAt: null } }); }
  const counts = await updateCampaignCounts(campaign._id); await Campaign.findByIdAndUpdate(campaign._id, { $set: { status: counts.emailPending ? 'sending' : 'completed', ...(counts.emailPending ? {} : { completedAt: new Date() }) } }); return { processed: claimed.length, batchNumber, counts };
}
...
app.post('/api/campaigns/:id/retry-failed', sendRateLimit, async (req, res) => { await CampaignRecipient.updateMany({ campaignId: req.params.id, channel: 'email', status: 'failed' }, { $set: { status: 'ready', failedAt: null, failureReason: null } }); const result = await processEmailBatch(await requireCampaign(req.params.id)); res.json(result); });
