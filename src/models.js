...
export const env = Object.freeze({ nodeEnv: process.env.NODE_ENV || 'development', port: number('PORT', 3000), mongoUri: process.env.MONGODB_URI || '', geminiApiKey: process.env.GEMINI_API_KEY || '', resendApiKey: process.env.RESEND_API_KEY || '', resendWebhookSecret: process.env.RESEND_WEBHOOK_SECRET || '', brevoApiKey: process.env.BREVO_API_KEY || '', emailName: process.env.EMAIL_NAME || 'SmartLocator', resendEmailFrom: process.env.RESEND_EMAIL_FROM || '', brevoEmailFrom: process.env.BREVO_EMAIL_FROM || '', emailFrom: process.env.EMAIL_FROM || '', emailReplyTo: process.env.EMAIL_REPLY_TO || '', appOrigin: process.env.APP_ORIGIN || '', rateLimitWindowMs: number('DISCOVERY_RATE_LIMIT_WINDOW_MS', 900000), rateLimitMax: number('DISCOVERY_RATE_LIMIT_MAX', 10), discoveryMaxAttempts: Math.min(number('DISCOVERY_MAX_ATTEMPTS', 4), 8), discoveryBatchSize: Math.min(number('DISCOVERY_BATCH_SIZE', 30), 50) });
...
const campaignSchema = new mongoose.Schema({
  ...
  emailTemplateId: { type: mongoose.Schema.Types.ObjectId, ref: 'OutreachTemplate', default: null }, whatsappTemplateId: { type: mongoose.Schema.Types.ObjectId, ref: 'OutreachTemplate', default: null }, emailProvider: { type: String, enum: ['resend', 'brevo'], default: 'resend' },
...
const recipientSchema = new mongoose.Schema({
  ...
  channel: { type: String, required: true, enum: ['email', 'whatsapp'] }, emailProvider: { type: String, enum: ['resend', 'brevo'], default: null }, recipient: { type: String, required: true, maxlength: 254 }, templateId: { type: mongoose.Schema.Types.ObjectId, ref: 'OutreachTemplate', required: true },
  status: { type: String, enum: ['pending', 'ready', 'sending', 'sent', 'failed', 'skipped', 'manual_sent'], default: 'ready', index: true },
  sentAt: Date, failedAt: Date, failureReason: { type: String, maxlength: 500 }, skipReason: { type: String, enum: ['whatsapp_not_registered', 'invalid_phone', 'wrong_number', 'already_contacted', 'do_not_contact', 'not_relevant', 'other'], default: null }, providerMessageId: { type: String, maxlength: 200 }, mailboxMessageId: { type: String, default: null, maxlength: 300 }, mailboxSubject: { type: String, default: null, maxlength: 500 }, mailboxText: { type: String, default: null, maxlength: 20000 }, mailboxPersistencePending: { type: Boolean, default: false, index: true }, attempts: { type: Number, default: 0 }, idempotencyKey: { type: String, required: true, unique: true, maxlength: 200 }, batchKey: { type: String, default: null, index: true }, batchNumber: { type: Number, default: null }, sendingLeaseExpiresAt: { type: Date, default: null }, providerAcceptanceUnknown: { type: Boolean, default: false }
}, { timestamps: true, versionKey: false });
recipientSchema.index({ campaignId: 1, leadId: 1, channel: 1 }, { unique: true }); recipientSchema.index({ status: 1, sentAt: -1 });
const activitySchema = new mongoose.Schema({
  leadId: { type: mongoose.Schema.Types.ObjectId, ref: 'Lead', required: true, index: true }, campaignId: { type: mongoose.Schema.Types.ObjectId, ref: 'Campaign', required: true, index: true }, channel: { type: String, enum: ['email', 'whatsapp'], required: true, index: true }, emailProvider: { type: String, enum: ['resend', 'brevo'], default: null, index: true }, templateId: { type: mongoose.Schema.Types.ObjectId, ref: 'OutreachTemplate', default: null }, recipient: { type: String, required: true }, recipientNormalized: { type: String, default: null, set: normalizeEmail }, subject: { type: String, default: null, maxlength: 200 }, status: { type: String, enum: ['sent', 'manual_sent', 'failed', 'skipped'], required: true, index: true }, sentAt: Date, failedAt: Date, failureReason: { type: String, maxlength: 500 }, skipReason: { type: String, enum: ['whatsapp_not_registered', 'invalid_phone', 'wrong_number', 'already_contacted', 'do_not_contact', 'not_relevant', 'other'], default: null }, providerMessageId: { type: String, maxlength: 200 }, nextFollowUpAt: { type: Date, default: null }
}, { timestamps: true, versionKey: false });
